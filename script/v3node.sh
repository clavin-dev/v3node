...
install() {
    bash <(curl -Ls "${GITHUB_RAW_BASE}/script/install.sh")
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls "${GITHUB_RAW_BASE}/script/install.sh") $version
    if [[ $? == 0 ]]; then
        echo -e "${green}更新完成，已自动重启 v3node，请使用 v3node log 查看运行日志${plain}"
        exit
    fi
...
config() {
    echo "v3node在修改配置后会自动尝试重启"
    vi /etc/v3node/config.json
    sleep 2
    restart
    check_status
    case $? in
        0)
            echo -e "v3node状态: ${green}已运行${plain}"
            ;;
        1)
            echo -e "检测到您未启动v3node或v3node自动重启失败，是否查看日志？[Y/n]" && echo
            read -e -rp "(默认: y):" yn
            [[ -z ${yn} ]] && yn="y"
            if [[ ${yn} == [Yy] ]]; then
               show_log
            ...
            ;;
        2)
            echo -e "v3node状态: ${red}未安装${plain}"
    esac
}

uninstall() {
    confirm "确定要卸载 v3node 吗?" "n"
    if [[ $? != 0 ]]; then
        if [[ $# == 0 ]]; then
            show_menu
        fi
...
start() {
    ...
    if [[ $? == 0 ]]; then
        echo ""
        echo -e "${green}v3node已运行，无需再次启动，如需重启请选择重启${plain}"
    else
        if [[ x"${release}" == x"alpine" ]]; then
        ...
        check_status
        if [[ $? == 0 ]]; then
            echo -e "${green}v3node 启动成功，请使用 v3node log 查看运行日志${plain}"
        else
            echo -e "${red}v3node可能启动失败，请稍后使用 v3node log 查看日志信息${plain}"
        fi
    fi
...
stop() {
    ...
    check_status
    if [[ $? == 1 ]]; then
        echo -e "${green}v3node 停止成功${plain}"
    else
        echo -e "${red}v3node停止失败，可能是因为停止时间超过了两秒，请稍后查看日志信息${plain}"
    fi
...
restart() {
    ...
    check_status
    if [[ $? == 0 ]]; then
        echo -e "${green}v3node 重启成功，请使用 v3node log 查看运行日志${plain}"
    else
        echo -e "${red}v3node可能启动失败，请稍后使用 v3node log 查看日志信息${plain}"
    fi
    if [[ $# == 0 ]]; then
...
status() {
    if [[ x"${release}" == x"alpine" ]]; then
        service v3node status
    else
        systemctl status v3node --no-pager -l
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
enable() {
    if [[ x"${release}" == x"alpine" ]]; then
        rc-update add v3node
    else
        systemctl enable v3node
    fi
    if [[ $? == 0 ]]; then
        echo -e "${green}v3node 设置开机自启成功${plain}"
    else
        echo -e "${red}v3node 设置开机自启失败${plain}"
    fi
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}v3node 取消开机自启成功${plain}"
    else
        echo -e "${red}v3node 取消开机自启失败${plain}"
    fi
...
show_log() {
    if [[ x"${release}" == x"alpine" ]]; then
        ...
        journalctl -u v3node.service -e --no-pager -f
    fi
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
update_shell() {
    wget -O "/usr/bin/${MANAGER_CMD}" -N --no-check-certificate "${GITHUB_RAW_BASE}/script/v3node.sh"
    if [[ $? != 0 ]]; then
        echo ""
...
check_enabled() {
    if [[ x"${release}" == x"alpine" ]]; then
        temp=$(rc-update show | grep v3node)
        if [[ x"${temp}" == x"" ]]; then
            return 1
        else
            return 0
        ...
        temp=$(systemctl is-enabled v3node)
...
check_uninstall() {
    ...
    if [[ $? != 2 ]]; then
        echo ""
        echo -e "${red}v3node已安装，请不要重复安装${plain}"
        if [[ $# == 0 ]]; then
            before_show_menu
        fi
        return 1
...
show_status() {
    ...
    case $? in
        0)
            ...
            ;;
        1)
            echo -e "v3node状态: ${yellow}未运行${plain}"
            show_enable_status
            ;;
        2)
            echo -e "v3node状态: ${red}未安装${plain}"
...
show_v3node_version() {
    echo -n "v3node 版本："
    /usr/local/v3node/v3node version
    echo ""
    if [[ $# == 0 ]]; then
        before_show_menu
    fi
...
show_usage() {
    ...
    echo "${MANAGER_CMD} update x.x.x - 安装 v3node 指定版本"
...
--- https://github.com/clavin-dev/v3node ---
  ${green}0.${plain} 修改配置
————————————————
  ${green}1.${plain} 安装 v3node
  ${green}2.${plain} 更新 v3node
  ${green}3.${plain} 卸载 v3node
————————————————
  ${green}4.${plain} 启动 v3node
  ${green}5.${plain} 停止 v3node
  ${green}6.${plain} 重启 v3node
  ${green}7.${plain} 查看 v3node 状态
  ${green}8.${plain} 查看 v3node 日志
————————————————
  ${green}9.${plain} 设置 v3node 开机自启
  ${green}10.${plain} 取消 v3node 开机自启
————————————————
  ${green}11.${plain} 查看 v3node 版本
  ${green}12.${plain} 升级 ${MANAGER_CMD} 维护脚本
  ${green}13.${plain} 生成 v3node 配置文件
 ...
 #后续更新可加入上方字符串中
    ...
    case "${num}" in
        ...
        11) check_install && show_v3node_version ;;
        12) update_shell ;;
...
if [[ $# > 0 ]]; then
    case $1 in
        ...
        "version") check_install 0 && show_v3node_version 0 ;;
